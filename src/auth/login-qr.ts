      ...
      switch (statusResponse.status) {
        ...
        case "expired": {
          ...
          if (!expiredRefreshResult.success) {
            activeLogins.delete(opts.sessionKey);
            return { connected: false, message: (expiredRefreshResult as { success: false; message: string }).message };
          }
          break;
        }
        case "verify_code_blocked": {
          ...
          if (!blockedRefreshResult.success) {
            activeLogins.delete(opts.sessionKey);
            return { connected: false, message: (blockedRefreshResult as { success: false; message: string }).message };
          }
          break;
        }
